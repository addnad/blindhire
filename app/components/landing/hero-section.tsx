...
const words = ["blind", "private", "fair", "encrypted"];
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12 py-32 lg:py-40">
        ...
        <div className="mb-12">
          <h1 className={`text-[clamp(3rem,12vw,10rem)] font-display leading-[0.9] tracking-tight transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <span className="block">Hiring that's</span>
        ...
        <div className="flex gap-16 marquee whitespace-nowrap">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex gap-16">
              {[
                { value: "100%", label: "data stays encrypted", company: "FHE" },
                { value: "0", label: "bias in matching", company: "BLINDHIRE" },
                { value: "2-party", label: "zero-knowledge match", company: "ZAMA" },
                { value: "on-chain", label: "verifiable results", company: "SEPOLIA" },
              ].map((stat) => (
