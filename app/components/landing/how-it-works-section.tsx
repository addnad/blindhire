...
const steps = [
  {
    ...
    code: `// Employer sets thresholds
const encYears = await encryptUint32(
  minYears,
  CONTRACT_ADDRESS,
  employerAddress
)

// Only ciphertext hits the chain
await postRole({
  _minYears: encYears.handle,
  _minYearsProof: encYears.proof,
})`,
  },
  {
    number: "II",
    ...
    code: `// Candidate encrypts their scores
const encScore = await encryptUint32(
  mySkillScore,
  CONTRACT_ADDRESS,
  candidateAddress
)

// Apply without revealing numbers
await applyForRole({
  roleId: BigInt(roleId),
  _score: encScore.handle,
  _scoreProof: encScore.proof,
})`,
  },
  {
    number: "III",
    title: "FHE contract computes the match",
    description: "The smart contract runs FHE comparisons on both ciphertexts. The match is computed entirely in encrypted space — no decryption mid-computation.",
    code: `// Inside the Solidity contract
ebool yearsOk = FHE.gte(
  candidate._years,
  role._minYears
)

ebool scoreOk = FHE.gte(
  candidate._score,
  role._minScore
)
...
FHE.allow(matched, candidate)`,
  },
  {
    number: "IV",
    title: "Each party decrypts their result",
    description: "FHE.allow() grants each wallet the right to decrypt only their own boolean. The employer never sees your scores. You never see their thresholds.",
    code: `// Candidate reveals their result
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        <div className="mb-16 lg:mb-24">
          ...
          <h2 className={`text-4xl lg:text-6xl font-display tracking-tight transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            Four steps.<br />
            <span className="text-background/50">Zero data exposure.</span>
          </h2>
        </div>
          ...
          <div className="lg:sticky lg:top-32 self-start">
            <div className="border border-background/10 overflow-hidden">
              <div className="px-6 py-4 border-b border-background/10 flex items-center justify-between">
                ...
                <span className="text-xs font-mono text-background/40">blindhire.ts</span>
              </div>
              ...
              <div className="px-6 py-4 border-t border-background/10 flex items-center gap-3">
                ...
                <span className="text-xs font-mono text-background/40">Sepolia · 0xa0bB4e71...dfD2e</span>
              </div>
            </div>
          </div>
        </div>
